...
function Events() {
    ...
    useEffect(()=>{
        ...
        setUrlPath(url);
        APIs.search(url).then(data => setEvents(data)).catch(error => alert(error))
    },[])
    return (
        <div className="event">
            ...
            <div className="event__header">
                <h2>{ events.length > 0 && `Tìm thấy ${events.length} bài viết`}</h2>
            </div>
            {events.map((event, index) => (
                <div className="event_item" key={index}>
                    ...
                    <div className="event_header">
                        <Link to={'/sự-kiện/'+ event.menuSlug+"/"+event.slug}>{event.title}</Link>
                    </div>
                    ...
                    <div className="event__content">
                        <div className="event__content_pic">
                            <Link to={'/sự-kiện/'+ event.menuSlug+"/"+event.slug}>
                        ...
                        <div className="event__content_text">
                            ...
                            <Link to={'/sự-kiện/'+ event.menuSlug+"/"+event.slug} style={{ textDecoration: 'none' }}>
                                <button type="submit">
                                    CONTINUE READING
                                    <p>
